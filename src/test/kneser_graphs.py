...
from coloring.examples import kneser_graph
...
def test_performance():
    ...
    file_path = 'test/data/kneser_graphs_performance.csv'
...
def test_accuracy():
    ...
    file_path = 'test/data/kneser_graphs_accuracy.csv'
    df = pd.DataFrame.from_dict(data)
    df.to_csv(file_path, sep='\t', encoding='utf-8')


if __name__ == '__main__':
    usage = 'usage: python3 kneser_graphs.py [performance | accuracy]'
    if len(sys.argv) < 2:
        print(usage)
