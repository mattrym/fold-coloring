...
def test_performance():
    ...
    for folds in fold_values:
        for param in params:
            print("Testing performance for param = {}".format(param))

            filename = "myciel{}.col".format(param)
            file_path = "test/instances/" + filename

            graph = load_dimacs(file_path)
            graph_name = "myciel{}".format(param)
    ...
    file_path = 'test/data/myciel_graphs_performance.csv'
...
def test_accuracy():
    ...
    for folds in fold_values:
        for param in params:
            print("Testing accuracy for param = {}".format(param))
    ...
    file_path = 'test/data/myciel_graphs_accuracy.csv'
    df = pd.DataFrame.from_dict(data)
    df.to_csv(file_path, encoding='utf-8')


if __name__ == '__main__':
    usage = 'usage: python3 myciel_graphs.py [performance | accuracy]'
    if len(sys.argv) < 2:
        print(usage)
